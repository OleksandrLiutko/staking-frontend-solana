...
export default function BakeCardLvlThree() {
    ...
    const createStatus = localStorage.getItem('createdStatus12') === 'created' ? true : false
    const createStatusPool = localStorage.getItem('createdPool12') === 'created'? true: false
    const connection = new anchor.web3.Connection(config.NET_RPC)

    const wallet = useWallet()

    const tokenMint = getTokenFromType('SI_TOKEN')
    // const isCreateState = localStorage.getItem('createdStatus5');
    // if (isCreateState !== null && isCreateState === 'created') {
    //     setCreateStatus(true);
    // }
    // const isCreatePool = localStorage.getItem('createdPool5');
    // if (isCreatePool !== null && isCreatePool === 'created') {
    //     setCreateStatusPool(true);
    // }
    ...
    const onStake = async () => {
        ...
        if (TVL + Number(stakeAmount) > STAKING[2].LIMIT) {
    ...
    const onUnstake = async () => {
        ...
        try {
            setUnStaking(true)
            await unstake(wallet, unStakeAmount, tokenMint, referral, STAKING_IDS[2])
        } catch (e) {
    ...
    return (
        <CardWrapper sx={{position:'relative'}}>
            <CardContent className="fact">
                ...
                >
                    ...
                    <Typography
                        ...
                        style={{ fontFamily: 'Montserrat' }}
                    >
                        {config.LEVEL_THREE_LOCK_TIME} months
                    </Typography>
                </UnderlinedGrid>
                <UnderlinedGrid
                    container
                        ...
                        <Box bgcolor="gray" padding={3} borderRadius={2}>
                            ...
                            <Box marginBottom={3}>
                                <Button
                                    variant="contained"
                                    fullWidth
                                    onClick={onWithdrawStakingReward}
                                    // hidden={!canShowSettings}
                                    className="custom-button"
                                >
                                    Withdraw
            ...
            <div style={{backdropFilter:'blur(8px)',position:'absolute',inset:'0',display:'flex',alignItems:'center',justifyContent:'center'}}>
                <Button
                    variant="contained"
                    fullWidth
                    className="coming-soon-button"
                    disabled={true}
                    style={{ fontFamily: 'Montserrat', fontSize: '48px', color: 'white' }}
                >
                    Coming Soon
                </Button>
            </div>
