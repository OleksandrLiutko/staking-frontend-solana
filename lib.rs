...
declare_id!("6WTuPyR7s9iymSJkheLRwyVjK1ctj6WZnqC8PaHWRpt3");
...
pub struct Stake<'info> {
    ...
    #[account(mut, seeds = [pool.key().as_ref(), referral.key().as_ref()], bump = user.bump, has_one = pool, has_one = authority)]
